...
bot = telebot.TeleBot('7715266575:AAEIW9CEugqzshwSgCV2rJmYFZKjJEiEwGc')
...
def process_attack_input(message):
    ...
    if len(command) == 3:
        ...
        if time > 380:
            response = "❗️𝗘𝗿𝗿𝗼𝗿: 𝘂𝘀𝗲 𝗹𝗲𝘀𝘀 𝘁𝗵𝗮𝗻 380 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❗️"
            bot.reply_to(message, response)
            return
...
def handle_buy_coins(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "💰 𝗗𝗠 𝗧𝗢 𝗕𝗨𝗬 𝗖𝗢𝗜𝗡𝗦 @NIRAJModzOwner")
